...
#include "lib/GBufferData.inc"
 ...
 {
   vec3 z=f.inv_direction*(v-1e-05-f.origin),s=f.inv_direction*(i+1e-05-f.origin),n=min(s,z),m=max(s,z);
   ...
   if(c)
     y=d(v-1e-05,i+1e-05,f.origin+f.direction*h),x=h;
   return c;
 }
 ...
 void G(inout float v,inout float x,float i,float f,vec3 s,float y)
 {
   v*=mix(2.4,3.4,f);
   v*=1.25;
 ...
 {
   float v;
   vec4 x=G(colortex7,texcoord.xy,false,8.,8.,vec2(1.,-1.),false,v);
   gl_FragData[0]=x;
 };
